...
def log_std_cauchy_target(z):
    """
    Standard d-dim Cauchy (independent coords) up to additive constant.

    z: shape (d,)
    returns: scalar
    """
    return -(1 + z * z).log().sum()


def log_std_cauchy_target_batch(z):
    """
    Standard d-dim Cauchy (independent coords) up to additive constant.
